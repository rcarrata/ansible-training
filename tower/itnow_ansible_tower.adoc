...
| Students | https://itnow.rhdemo.io ||
| Identity Manager | https://idm.rober.labs | 3.123.32.118 |
...
| Control Node | https://<student>.itnow.rhdemo.io ||
| Managed Node1 | node1.example.com ||
| Managed Node2 | node2.example.com ||
| Managed Node3 | node3.example.com ||
|===

TIP: Your lab environment will get a unique *<student_id>*. You will be able to SSH into control
node using the following for SSH access http://itnow.rhdemo.io/ from here you need to SSH into the other hosts to run tasks on the commandline.
...
* LDAP BIND DN
----
uid=admin,cn=users,cn=compat,dc=rober,dc=labs
----
* LDAP BIND PASSWORD
----
redhat00
...
----
[
 "cn=users,cn=accounts,dc=rober,dc=labs",
 "SCOPE_SUBTREE",
 "(uid=%(user)s)"
]
----
...
----
[
 "cn=groups,cn=accounts,dc=rober,dc=labs",
 "SCOPE_SUBTREE",
...
{
 "is_superuser": [
  "cn=tower.admins,cn=groups,cn=accounts,dc=rober,dc=labs"
 ],
 "is_system_auditor": [
  "cn=tower.auditors,cn=groups,cn=accounts,dc=rober,dc=labs"
 ]
}
----
...
----
{
 ...
 "Operators": {
  "organization": "Default",
  "users": [
   "cn=tower.operators,cn=groups,cn=accounts,dc=rober,dc=labs"
  ],
  "remove": true
 },
 "Linux": {
  "organization": "Default",
  "users": [
   "cn=tower.linux,cn=groups,cn=accounts,dc=rober,dc=labs"
  ],
  "remove": true
 }
}
...
* Access to idM Web ui https://idm.rober.lab/ipa/ui/#/e/user/search (ip address 3.123.32.118, add
* to your local /etc/hosts file "3.123.32.118 idm.rober.labs")
** User: admin
** Password: redhat00
...
* Access to idM Web ui https://idm.rober.lab/ipa/ui/#/e/user/search (ip address 3.123.32.118, add
* to your local /etc/hosts file "3.123.32.118 idm.rober.lab")
** User: admin
** Password: redhat00
